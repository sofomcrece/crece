class ContratoPdf < Prawn::Document
  ...
  def text_content
    ...
    text "CUARTA. “EL ACREDITADO” se obliga a pagar a “LA ACREDITANTE” intereses ordinarios sobre el monto del crédito a razón de una tasa fija del #{'%.2f' % (@credit.product.taza_de_interes_ordinaria / 1.16)}% (por ciento) anual más IVA (Impuesto al Valor Agregado). Convienen las partes en que el interés que se aplique al importe financiero se calculará a partir de la fecha de firma del presente contrato, los mismos se pagarán en las fechas indicadas en la tabla de amortización. El pago de los intereses no podrá ser exigido por adelantado, sino únicamente por períodos vencidos.",:size =>10,:align => :justify
    text " "
    ...
    sucesivo CRECE PRESTACIONES) en su calidad de Acreedor, en   sus   oficinas   con   Domicilio: #{Conf.last.domicilio}, y/o en la cuenta bancaria #{Company.last.cuenta_bacaria}, CLABE #{Company.last.cuenta_bacaria}, BANCO	#{Company.last.banco}, la cantidad total de #{Dinero.to_money((@credit.monto_solicitud.to_f + (@credit.monto_solicitud.to_f * (@credit.taza_ord/100))))} (#{Dinero.to_words((@credit.monto_solicitud.to_f + (@credit.monto_solicitud.to_f * (@credit.taza_ord/100))))} PESOS 00/100 M.N.), correspondiente al principal, a los intereses ordinarios y   la   comisión   por   apertura,   mediante   pagos   conforme   a   la   tabla   de   amortización debidamente firmada por el DEUDOR.",:size =>8,:align => :justify
    text " "
    text "El presente Pagaré es causal y documental del Contrato de Crédito Simple de fecha #{@credit.fecha_en_español}, celebrado entre el DEUDOR del presente Pagaré, en calidad de ACREDITADO, y el Acreedor del presente Pagaré, en calidad de ACREDITANTE del Crédito Personal otorgado (en lo sucesivo el “Crédito”).",:size =>8,:align => :justify
    text " "
    ...
    text " "
    text "Para toda controversia o litigio de cualquier tipo en relación con el presente   Pagaré,   el   DEUDOR   está   de   acuerdo   en   someterme expresamente a la jurisdicción y competencia de los Tribunales de la ciudad de #{Company.last.municipio},#{Company.last.estado}, por lo tanto, el DEUDOR renuncia al fuero territorial que por cualquier causa pudiere corresponderle, ya sea en lo presente o en lo futuro.",:size =>8,:align => :justify
    text " "
