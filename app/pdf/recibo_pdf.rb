class ReciboPdf < Prawn::Document
  ...
  def text_content
    ...
    @tickets.each do |ticket|  
      ...
      draw_text  "RECIBO DE : #{ticket.payment.credit.nombre_completo_deudor} ",:size=>9, :at => [70,670+des]
      draw_text  "LA CANTIDAD DE:  #{Dinero.to_money(ticket.cantidad)}",:size=>9, :at => [70,660+des]
      draw_text  "                     (#{(I18n.with_locale(:es) { (ticket.cantidad).to_i.to_words }).upcase } PESOS 00/100 M.N.)",:size=>9, :at => [70,650+des]
      ...
      draw_text  "ATRASADO ACTUAL DE: #{Dinero.to_money at} ",:size=>9, :at => [70,620+des]
      ...
      draw_text  "LA CANTIDAD DE:  #{Dinero.to_money(ticket.cantidad)}",:size=>9, :at => [70,660+des]
      draw_text  "                     (#{I18n.with_locale(:es) { (ticket.cantidad).to_i.to_words }} PESOS /100 M.N.)",:size=>9, :at => [70,650+des]
      draw_text  "POR CONCEPTO DE: PAGO O ABONO AL CARGO QUINCENAL #{ticket.payment.recibo} CORRESPONDIENTE AL CREDITO",:size=>9, :at => [70,640+des]
      draw_text  "                  OTORGADO EL #{ticket.payment.credit.fecha}",:size=>9, :at => [70,630+des]
