class CaratulaPdf < Prawn::Document
  ...
  def text_content
     ...
     draw_text  "#{'%.2f' % @credit.cat_sin_iva}%",:size=>9, :at => [55 ,619]
      ...
      draw_text  "Dia #{Payout.array_to_text(@credit.product.payout.getDays)}",:size=>9, :at => [280,579]
      draw_text  "Dia #{Payout.array_to_text(@credit.product.payout.getFlow)} de cada #{@credit.product.payout.type_payout==0?"semana":@credit.product.payout.type_payout==1?"mes":"año"}",:size=>9, :at => [260,570]
