class Auxiliar < ActiveRecord::Base
    ...
    def self.seguimiento_por_creditos_guardados(credits,fecha)
      ...
      credits.each do |credit|
        ...
        xultpago = Ticket.joins(:payment=>:credit).where("credits.id = ? and tickets.status = ?",credit.id,0).order("tickets.updated_at").last
        next if xadeudo == 0 and xultpago.tickets.updated_at.to_date > fecha.to_date
